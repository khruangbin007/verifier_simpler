...
**Choosing the final outputs.** The sheet `Model_Implementation_Map` lists every function of the package, the model's final outputs first and numbered `01`, `02`: for each, what code proposes and why, the functions it calls and those that call it, and whether any final output reaches it. Code proposes as a final output a function nothing in the package calls that is exported or that the package's tests or vignettes call (step 05a); the functions no final output reaches are marked so. The yellow column **Final output (your decision)** has a drop-down of `yes` and `no`: `yes` makes any function a final output, even one other functions call; `no` takes one away; an empty cell leaves code's proposal, and emptying a cell you filled before withdraws that decision. Cell 4 reads the column back by function name when you confirm the outline, records each change with your id in a hash chain, and says which final outputs the run will map. Anything else written in the column is ignored and cell 4 says so. Where neither code nor you names one, every function nothing in the package calls stands in, so the map always has a top.
