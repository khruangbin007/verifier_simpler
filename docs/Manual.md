...
**The Model Implementation Map.** The sheet `Model_Implementation_Map` shows how the model computes what it returns, from each final output down to the rawest inputs. Each final output is a top row, `01`, `02`; under each row are the steps it is computed from, numbered after it (`01.02`, `01.02.03`), so that sorting the Map ID column gives the tree back. The rows are grouped: each parent is a summary row above its members, and the + and − at the left of the sheet open and close a branch. Excel groups eight levels deep; a deeper step keeps its ID and is indented further, but opens and closes with the eighth level. A step is a value a function sets, a column a dplyr verb creates, or a call of another function, which is entered with the arguments that call gives it, so under `cond_pd()` inside `capital_k` stands what `capital_k` gave it there, and a parameter the call leaves out shows its default. A step already shown in the same calling context appears again as a single row, *see 01.02.03*. A function that calls itself is a row marked as a loop, which still shows what the call is given. The leaves are the raw inputs: an argument of the final output, a column of the data given, a stored table, a file, a hard-coded number, a name from outside the package.

Each row carries: the step, in the AI's plain words where the Namer gave them (in quotation marks) and otherwise as the code names it; its role; the function it is about; its output variable and input variables; its model unit and the code as written; its concepts; the methodology and documentation linked to it, with how; the checks, status and flagged items of its unit, shown once, on the unit's first row; how the step was established — parsed from the code, what a call gives which parameter, or the AI's tracing with the code it quoted, or an open gap; and the model units related to a function — its roxygen, help pages, tests, and statements no other row shows. Three branches follow the final outputs, so that nothing falls out: **90**, the model units no final output reaches; **91**, the methodology no step implements — passages that state a number, formula or rule, that the AI judge linked to no step of the map, and whose numbers are not all values the map uses (hard-coded in a step, or held in a table or file it reads); and **92**, the documentation describing nothing in the map — linked to no step and naming none of the model's concepts.

**Choosing the final outputs.** Code proposes as a final output a function nothing in the package calls that is exported or that the package's tests or vignettes call (step 05a); the functions no final output reaches are in branch 90. The yellow column **Final output (your decision)** has a drop-down of `yes` and `no`: `yes` makes any function a final output, even one other functions call; `no` takes one away; an empty cell leaves code's proposal. A function has one decision however many rows it appears on: the recorded decision is shown on its first row, `yes` and `no` written on its rows at once are both ignored and said, and emptying every row of a function you decided before withdraws that decision. Cell 4 reads the column back by function name when you confirm the outline, records each change with your id in a hash chain, and says which final outputs the run will map. Anything else written in the column is ignored and cell 4 says so. Where neither code nor you names one, every function nothing in the package calls stands in, so the map always has a top.
