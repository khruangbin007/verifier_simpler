...
`map_hops_max` (default 8): the most turns the Tracer takes on one gap. `map_calls_max` (default 200): the most questions the map's agents ask in one run, the Tracer's and the Namer's together. `map_granularity` (default `statement`): which steps the Namer names — `statement` for every value, column and function, `function` for functions only. `map_with_ai` (default true): whether the map's agents are asked at all.
