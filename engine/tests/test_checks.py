...
def compared(code, stated, defaults=None, bridge=None):
    prepared = review.prepare_alignment(reading.parse_formula(code, NOTATION), reading.parse_formula(stated, NOTATION), defaults or {}, bridge or {})
    return review.compare_formulas(prepared, SETTINGS, {}), prepared


class ValueRule(unittest.TestCase):
    def test_the_table_of_the_rule(self):
        number = core.parse_number
        ...
        for stated, other, exact, expected in cases:
            self.assertEqual(review.compare_values(number(*stated), number(*other), exact)[0], expected, (stated, other))

    def test_the_rounding_convention_is_named_only_when_the_two_differ(self):
        outcome, note = review.compare_values(core.parse_number("2.5"), core.parse_number("2.45"), exact=True)
        self.assertEqual((outcome, note), ("agrees at stated precision", "under round-half-up"))
        self.assertEqual(review.compare_values(core.parse_number("0.04"), core.parse_number("0.0351"), exact=True)[1], "")
...
class FormulaComparison(unittest.TestCase):
    ...
    def test_a_default_is_put_in_only_where_the_stated_formula_shows_that_number(self):
        ...
        self.assertEqual((result["outcome"], result["undecided_reason"]), (core.CHECK_UNDECIDED, core.UNDECIDED_REASONS[2]))
    ...
    def test_symbols_left_over_make_the_check_undecided_never_agreeing(self):
        ...
        self.assertEqual((result["outcome"], result["undecided_reason"]), (core.CHECK_UNDECIDED, "symbols could not be aligned"))

    def test_points_outside_a_domain_are_not_counted_and_too_few_valid_points_is_undecided(self):
        tree = reading.parse_formula("y = ln(x)", NOTATION)
        self.assertIsNone(review.evaluate(tree.args[1], {"x": -1.0}))
        result, _ = compared("y = sqrt(x - 100)", "Y = sqrt(x - 100) + 0")
        self.assertIn(result["outcome"], ("agrees", core.CHECK_UNDECIDED))
        prepared = review.prepare_alignment(reading.parse_formula("y = sqrt(x - 100)", NOTATION), reading.parse_formula("Y = sqrt(x - 100.5)", NOTATION), {}, {})
        result = review.compare_formulas(prepared, SETTINGS, {})
        self.assertEqual((result["outcome"], result["undecided_reason"]), (core.CHECK_UNDECIDED, "too few valid sample points"))

    def test_stored_values_are_used_row_by_row(self):
        tree = reading.parse_formula("y = rate * 2", NOTATION)
...
class TablesRulesAndProse(unittest.TestCase):
    ...
    def test_cells_are_compared_by_key_and_rows_on_one_side_only_are_listed(self):
        result = review.reconcile(self.OURS, self.THEIRS, STOP, exact=True)
        self.assertEqual(result["outcome"], "differs")
    ...
    def test_columns_without_usable_headers_are_matched_by_their_values(self):
        ...
        result = review.reconcile(ours, dict(theirs, header=["segment", "Value in force"]), STOP, exact=True)
    ...
    def test_a_percent_header_converts_the_stated_values(self):
        ...
        self.assertEqual(review.reconcile(self.OURS, theirs, STOP, exact=True)["outcome"], "agrees")
    ...
    def test_a_table_in_long_form_is_laid_over_a_printed_grid(self):
        ...
        result = review.reconcile(long_form, grid, STOP, exact=True)
    ...
    def test_tables_that_code_cannot_lay_over_each_other_use_the_validated_ai_mapping(self):
        ...
        self.assertEqual(review.reconcile(ours, theirs, STOP, exact=True)["outcome"], "could not be compared")
        mapped = review.reconcile(ours, theirs, STOP, exact=True, ai_map={"columns": [("seg", "Segment"), ("floor_value", "LGD floor")], "key": ("seg", "Segment")})
    ...
    def test_rules_are_recognised_by_generic_phrases(self):
        ...
        self.assertEqual([(kind, number["as_written"]) for kind, number, _ in review.stated_rules(chunk)], [("floor", "0.03%"), ("cap", "20%")])
    ...
    def test_a_floor_held_as_the_default_of_an_argument_is_found_by_code_inspection(self):
        ...
        members = [u for u in units if u["kind"] in (core.KIND_FUNCTION, core.KIND_FORMULA)]
        number = core.find_numbers("never below 0.03%")[0]
        self.assertIn("maximum with 0.03%", review.rule_in_trees("floor", number, members))
        self.assertEqual(review.rule_in_trees("cap", number, members), "")
    ...
    def test_prose_numbers_are_compared_only_with_a_stated_number_that_found_no_equal(self):
        ...
        lines, differs = review.prose_value_lines("cond_pd evaluates the conditional probability at the 99.5th percentile of the systematic factor.", stated, STOP, set(), "documentation")
        self.assertTrue(differs)
        stated = [("C-0003", "The dose rate (R) is 15 milligrams per kilogram.")]
        lines, differs = review.prose_value_lines("It uses the dose rate of 15 milligrams per kilogram and never returns more than 4000 milligrams.", stated, STOP, set(), "documentation")
...
class SeededSample(unittest.TestCase):
    ...
    def test_the_flipped_sign_in_cond_pd_is_caught_with_a_counterexample(self):
        ref = self.ref_of(core.KIND_FUNCTION, "cond_pd")
        self.assertEqual(self.status[ref]["status"], core.ST_DIFFERS)
        ...
        self.assertTrue([i for i in self.items if ref in i["unit_refs"] and i["category"] == core.CAT_CODE_DIFFERS])

    def test_all_six_seeded_differences_are_flagged_in_an_expected_category(self):
        expected = ((core.KIND_FUNCTION, "cond_pd", core.CAT_CODE_DIFFERS), (core.KIND_FUNCTION, "asset_correlation", core.CAT_CODE_DIFFERS),
                    (core.KIND_FUNCTION, "floor_pd", core.CAT_CODE_DIFFERS), (core.KIND_TABLE, "lgd_floors", core.CAT_VALUE_DIFFERS),
                    (core.KIND_ROXYGEN, "cond_pd", core.CAT_PKGDOC_VS_CODE))
        ...
        stale = [i for i in self.items if i["category"] == core.CAT_DOC_VS_CANON and "99.5th percentile" in i["observed"]]
    ...
    def test_an_equation_given_as_an_image_is_never_skipped(self):
        ...
        self.assertTrue([i for i in self.items if image["ref"] in i["unit_refs"] and i["category"] == core.CAT_NOT_READ])
        tree, source, reason = review.stated_formula(image, {})
    ...
    def test_the_identity_not_clean_if_and_only_if_an_item_names_the_unit(self):
        ...
        for ref, status in self.status.items():
            ...
            self.assertEqual(status["clean"], status["status"] in core.CLEAN_STATUSES)
    ...
    def test_a_broken_identity_stops_the_run_and_names_the_part(self):
        ...
        items = [core.FlaggedItem(i["item_id"], i["category"], i["concerns"], tuple(i["unit_refs"]), i["item"], i["observed"]) for i in self.items]
    ...
    def test_every_status_was_decided_by_a_rule_of_the_ordered_list(self):
        listed = {(corner, name) for corner, name, _, _ in review.STATUS_RULES}
        for status in self.status.values():
            self.assertIn((status["corner"], status["decided_by_rule"]), listed)
        import inspect
        source = inspect.getsource(review.model_unit_outcome) + inspect.getsource(review.doc_unit_outcome) + inspect.getsource(review.account_coverage)
        for corner, name, _, _ in review.STATUS_RULES:
    ...
    def test_items_carry_a_path_side_by_side_texts_and_a_neutral_next_step(self):
        item = next(i for i in self.items if i["category"] == core.CAT_VALUE_DIFFERS)
        ...
        self.assertFalse(core.has_banned_wording(item["suggested_next_step"]))
...
class CleanSamples(unittest.TestCase):
    def test_the_clean_baseline_flags_none_of_the_units_listed_as_clean(self):
        ...
        for sample in ("A_minimal", "F_capital", "D_dosing"):
            ...
            clean = {s["unit_ref"]: s["clean"] for s in runner.open_store(paths, settings).read("unit_status")}
