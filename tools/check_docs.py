...
def problems():
    ...
    for step in runner.load_pipeline()["steps"]:                             # 3. skills
    ...
    for word in core.CLEAN_STATUSES + core.NOT_CLEAN_STATUSES + core.CATEGORIES + core.DECISION_WORDS:
        if word not in manual:
            found.append("'%s' is not explained in the manual" % word)
    for name in runner.DEFAULT_SETTINGS:
        if not build_manual.SETTING_NOTES.get(name):
            found.append("setting '%s' has no explanation in build_manual.SETTING_NOTES" % name)
    if core.has_banned_wording(re.sub(r"<!-- wording-policy-sentence -->.*?<!-- end -->", "", manual, flags=re.S)):
        found.append("the manual uses the word '%s'" % core.has_banned_wording(re.sub(r"<!-- wording-policy-sentence -->.*?<!-- end -->", "", manual, flags=re.S)))
